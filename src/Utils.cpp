...
namespace Utils
{
	...
	static void _makeFolders(
		...
		const std::function<void(bool b)> &open,
		...
		for (auto &entry : paths) {
			...
			auto fileStr = filePath.filename().string();
	...
	std::string openFileDialog(const std::string &title, const std::string &basePath, const std::vector<std::pair<std::string, std::string>> &patterns, bool overWriteWarning, bool mustExist, bool folder)
	{
		sf::RenderWindow window{{500, 300}, title.c_str(), sf::Style::Titlebar};
		tgui::Gui gui{window};
		std::string result;
		...
		while (!std::filesystem::is_directory(currentPath)) {
			...
			startText += currentPath.filename().string();
			currentPath = currentPath.parent_path();
		}

		gui.loadWidgetsFromFile("gui/open_file_dialog.gui");
		...
		std::function<void(bool b)> open = [&gui, &result, &window, path, box, file, &currentPath, panel, &open, mustExist, overWriteWarning, folder](bool b){
			...
			if (std::filesystem::is_directory(result) && (!folder || !b)) {
			...
			if (overWriteWarning && std::filesystem::exists(result)) {
				...
				win->loadWidgetsFromFile("gui/overwrite_warning.gui");
		...
		auto openBtn = gui.get<tgui::Button>("Open");

		openBtn->connect("Clicked", open, true);

		if (overWriteWarning)
			openBtn->setText("Save");

		if (folder) {
			openBtn->setText(openBtn->getText() + " a folder");
			openBtn->setSize(openBtn->getSize().x + 50, openBtn->getSize().y);
		}

		for (auto &pair : patterns)
			box->addItem(pair.second, !folder ? pair.first : "folder");
		if (!folder)
	...
	std::string openFolderDialog(const std::string &title, const std::string &basePath, const std::vector<std::pair<std::string, std::string>> &patterns, bool overWriteWarning, bool mustExist)
	{
		return openFileDialog(title, basePath, patterns, overWriteWarning, mustExist, true);
	}
	...
	tgui::ChildWindow::Ptr openWindowWithFocus(tgui::Gui &gui, tgui::Layout width, tgui::Layout height)
	{
		...
		const bool tabUsageEnabled = gui.isTabKeyUsageEnabled();
		auto closeWindow = [&gui, window, panel, tabUsageEnabled](std::weak_ptr<tgui::ChildWindow> w){
			gui.remove(w.lock());
			gui.remove(panel);
			gui.setTabKeyUsageEnabled(tabUsageEnabled);
		};

		panel->connect("Clicked", closeWindow, std::weak_ptr(window));
		window->connect({"Closed", "EscapeKeyPressed"}, closeWindow, std::weak_ptr(window));
		return window;
	}
	...
	tgui::ChildWindow::Ptr makeColorPickWindow(tgui::Gui &gui, const std::function<void(Color color)> &onFinish, Color startColor)
	{
		...
		window->loadWidgetsFromFile("gui/color.gui");
		...
		for (auto &widget : window->getWidgets())
			if (widget->getWidgetName().substring(0, strlen("Button")) == "Button") {
				auto button = window->get<tgui::Button>(widget->getWidgetName());
	...
	tgui::ChildWindow::Ptr makeSliderWindow(tgui::Gui &gui, const std::function<void(float value)> &onFinish, float defaultValue, float min, float max, float step)
	{
		...
		window->loadWidgetsFromFile("gui/slider.gui");
	...
	Color::Color(tgui::Color color) :
		Color(static_cast<sf::Color>(color))
	{
	}

	Color::operator tgui::Color()
	{
		return {
			this->r,
			this->g,
